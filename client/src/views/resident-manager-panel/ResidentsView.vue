...
<template>
    ...
    <ul class="grid w-full gap-4 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      <ResidentCard
      :resident="resident"
      :class="{ 'animate-scale-up border-2 border-blue-400': matchSearch(resident, searchText, fieldsToSearch) && searchText.length > 2, 'hidden': !matchSearch(resident, searchText, fieldsToSearch) && hideNoResults && searchText.length > 2 }"

...
import { getOwnResidence, getResidentByResidence } from "@/composables/useResidence";
...
const fieldsToSearch = ["firstName", "lastName", "identifier"];
...
async function getResidence() {
  ...
  if (!data.value?.data) {
    toast.addToast({ type: ToastType.ERROR, message: "No estás asignado a ninguna residencia" });
    return;
  }

  residence.value = data.value.data;
}
...
onMounted(async () => {
  ...
  const { data } = await getResidentByResidence(residence.value.uuid);

<template>
  ...
  <ul class="mt-4 grid gap-4 md:grid-cols-3">
    <li class="rounded-lg bg-pasadita-blue-4 transition-all hover:bg-pasadita-blue-6 active:scale-95" v-for="route, index in availableRoutes" :key="index">
      <a :href="route.path" class="flex flex-col p-2 text-pasadita-blue-1">
        <span class="text-xl font-semibold capitalize">{{ route.name?.toString().replace("-", " ") }}</span>

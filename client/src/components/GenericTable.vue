...
const props = defineProps<{ table: GenericTableType[], searchTerm?: string, hideNoResults? : boolean }>()
...
const rowMatchesSearch = (rowIndex: number): boolean => {
    ...
    return props.table.some(column => {
        ...
        return typeof entry === 'string' && entry.toLocaleLowerCase().includes(props.searchTerm?.toLocaleLowerCase()!);
    });
};
    ...
    <div class="flex w-min min-w-full flex-col overflow-hidden rounded-xl border border-blue-200 bg-blue-100">
        ...
        <table class="min-w-full">
            <thead class="bg-blue-100 text-left text-blue-500">
                <tr>
                    <th class="border-r border-r-blue-300 p-2 font-medium last:border-r-0" v-for="(item, index) in props.table" :key="index">
            ...
            <tbody class="text-blue-400">
                <tr v-for="(i) in props.table.at(0)?.items.length" :key="i" 
                    :class="{'bg-blue-400 font-bold text-white': rowMatchesSearch(i - 1), 'transition-all odd:bg-shades-100 even:bg-white hover:bg-blue-200': !rowMatchesSearch(i - 1), 'hidden' : !rowMatchesSearch(i - 1) && hideNoResults}">

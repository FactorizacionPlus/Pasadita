...
<template>
  <div
    class="flex w-min min-w-full flex-col overflow-hidden rounded-xl border border-blue-200 bg-blue-100"
  >
    <slot />
    <table class="min-w-full">
      <thead class="bg-blue-100 text-left text-blue-500">
        <tr>
          <th
            class="border-r border-r-blue-300 p-2 font-medium last:border-r-0"
            v-for="(item, index) in props.table"
            :key="index"
          >
      ...
      <tbody class="text-blue-400">
        <tr
          v-for="i in props.table.at(0)?.items.length"
          :key="i"
          :class="{
        ...
        >
          <td
            class="border-r border-r-blue-300 p-2 last:border-r-0"
            v-for="j in props.table.length"
            :key="j"
          >

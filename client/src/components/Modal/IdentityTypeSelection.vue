<template>
    <div class="flex flex-col gap-1">
        ...
        <div class="grid w-full grid-cols-2 gap-2">
            <button v-for="(item, index) in identityTypes" :key="index" type="button"
                class="group grid h-32 place-items-center rounded-lg border border-blue-200 transition-all active:scale-95"
                :class="selection != item.type ? 'bg-white hover:bg-shades-100 text-blue-400' : 'bg-blue-400 text-white'"
                @click="handleSelection(item.type)">
                <component class="transition-transform duration-500 group-hover:scale-105" :is="item.icon"/> 
            </button>
        </div>
    </div>
</template>
...
const selection = ref<identifierType>( props.identityType != undefined ? props.identityType : identityTypes[0].type);

<template>
  <nav
    ...
    <div class="flex justify-between gap-4 md:flex-col md:justify-start">
      ...
      <button :class="(isNavActive ? 'bg-pasadita-blue-2 text-white hover:!bg-pasadita-blue-3 hover:text-white' : '')"
        @click="toggleNav"
    ...
    <div :class="isNavActive ? 'block' : 'hidden'" class="mt-4 h-full flex-col justify-between gap-1 md:flex">
      <RouterLink :key="index" v-for="(route, index) in routes"
        :class="currentPath == (ADMIN_ROUTE + route.link) ? 'bg-pasadita-blue-2 text-white hover:!bg-pasadita-blue-3 hover:text-white' : ''"

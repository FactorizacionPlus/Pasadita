<template>
  <nav class="sticky top-4 z-10 flex flex-col rounded-lg bg-white p-2 shadow-header transition-all md:relative md:top-0 md:p-4" :class="isFullNav ? 'md:w-[360px]' : 'md:w-[80px]'">
    <div class="relative flex justify-between gap-4 md:flex-col md:justify-start">
      ...
      <button @click="() => { isFullNav = !isFullNav }"
        ...
        <VueFeather :class="isFullNav && 'rotate-180'" type="chevron-right" class="w-4 transition-transform"
          stroke-width="2.5"></VueFeather>
      </button>
    </div>
    <div :class="isNavActive ? 'block' : 'hidden'" class="mt-4 h-full flex-col justify-between gap-1 md:flex">


      <div :class="isUserPresetActive && 'bg-shades-100'" class="mb-4 flex flex-col gap-2 rounded-lg transition-colors md:mb-0">
        
        <button @click="() => { isUserPresetActive = !isUserPresetActive }"
          class="flex items-center gap-1 rounded-lg p-2 text-blue-400 transition-all hover:bg-shades-100 active:scale-95">
          <UserImage :class="!isFullNav && 'md:!size-8 md:mx-auto'" class="size-12" image="https://www.github.com/davidquintr.png" />
        ...
        <div class="p-2 pt-0" v-if="isUserPresetActive">
          <RouterLink :key="index" v-for="(route, index) in userPresets"
